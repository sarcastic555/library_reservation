...
class ReserveBookNumCalculator:
  ...
  def compare_reserve_num_to_reserve_list_size(self):
    ...
    self.shortwait_reserve_book_num = min(self.shortwait_reserve_book_num, self.shortwait_num_in_want_reserve_list)
    self.longwait_reserve_book_num = min(self.longwait_reserve_book_num, self.longwait_num_in_want_reserve_list)
  ...
  def consider_maximum_reserve_num_per_day(self):
    ...
    self.shortwait_reserve_book_num = min(self.shortwait_reserve_book_num, self.__class__.reservenum_per_day)
    self.longwait_reserve_book_num = min(self.longwait_reserve_book_num, self.__class__.reservenum_per_day)
...
class ReserveBookInfoEvaluator:
  ...
  def print_info(self):
    ...
    print(f"Shortwait reservation list size = {self.want_reserve_list_info.shortwait_reservelist_num}")
    print(f"Longwait reservation list size = {self.want_reserve_list_info.longwait_reservelist_num}")
    ...
    if (booklist_num == 0):
      ...
      for i in range(reserve_num):
        ...
        while j < self.reading_info.nowreading_num:
          ...
          if int(float(want_reserve_list_df.iloc[index_candidate]['13桁ISBN'])) == int(self.reading_info.df.iloc[j]['ISBN']):
            continue
          j+=1
  ...
  def get_reserve_isbn_list_shortwait(self, reserve_num):
    return self.get_reserve_isbn_list(self.want_reserve_list_info.shortwait_reservelist_df, reserve_num)

  def get_reserve_isbn_list_longwait(self, reserve_num):
    return self.get_reserve_isbn_list(self.want_reserve_list_info.longwait_reservelist_df, reserve_num)
