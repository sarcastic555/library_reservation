...
def options() -> argparse.ArgumentParser:
  ...
  parser.add_argument('--no_reservation_booklist_file',
                      help='Input path to no reservation book list.',
                      default='list/no_reservation.csv')
  parser.add_argument('--has_reservation_booklist_file',
                      help='Input path to with reservation book list.',
                      default='list/has_reservation.csv')
  parser.add_argument(
      '--shortwait_reserve_book_num_file',
      help='Input path to file where shortwait reserve book number info is recorded.',
      default='result/shortwait_reserve_size.csv')
  parser.add_argument(
      '--longwait_reserve_book_num_file',
      help='Input path to file where longwait reserve book number info is recorded.',
  ...
  parser.add_argument('--output_shortwait_reserve_list',
                      help='Output path to shortwait reserve book list.',
                      default='list/shortwait_reserve_list.csv')
  parser.add_argument('--output_longwait_reserve_list',
                      help='Output path to longwait reserve book list.',
...
def get_reserve_isbn_list(want_reserve_list_df: pd.DataFrame, nowlending_df: pd.DataFrame,
